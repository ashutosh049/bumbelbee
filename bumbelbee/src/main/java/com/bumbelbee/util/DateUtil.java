...
public final class DateUtil {
	...
	public static boolean isBetween(Date start, Date end, Date compare) {
		...
		return ((start.getTime() <= compareTime) && (compareTime <= end
				.getTime()));
	}
	...
	public static String translateDate(String timestampStr, String fromFormat,
			String toFormat) {
	...
	public static Date[] createEquidistantDates(Calendar reference,
			final int periods, final TimeUnit sampleUnit, int sampleRate) {
		int position = 7;
 		if (reference == null) {
			reference = Calendar.getInstance();
		}
